...
class Agent:
    def __init__(self, id):
        ...
        # tuning parameters
        self.buying_probability = 1
        self.auction_price_factor = 1
        self.trade_probability = 1
        self.jail_decision_probability = 1
        self.build_house_probability = 1
    ...
    def buyProperty(self, state):
        ...
        if self.me["cash"] >= board[self.game_state.current_position]["price"]:
            ...
            if self.give_probability_decision(self.buying_probability):
                return True
        return False
    ...
    def respondTrade(self, state):
        ...
        if self.me["cash"] + cash_offer >= cash_req:
            if self.give_probability_decision(self.trade_probability):
                return True
        return False
    ...
    def jailDecision(self, state):
        ...
        if self.me["cash"]>=50:
            if self.give_probability_decision(self.jail_decision_probability):
                return ("P" ,)
        return ("R", )
        ...
        for each in self.get_all_owned():
            ...
            if all(x in monopoly for x in self.get_all_owned(person=self.me)):
                if total_budget-board[each]["build_cost"] >=0:
                    ...
                    break
        if build_house_locations and total_budget >= 0 and self.give_probability_decision(self.build_house_probability):
    ...
    def give_probability_decision(self, prob):
        return round(random.uniform(0, 1), 2)<=round(prob, 2)
